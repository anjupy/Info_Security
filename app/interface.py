...
def run_interface():
    ...
    if uploaded_file is not None:
        ...
        encrypted_data = encrypt_data(file_content.encode("utf-8"))  # Ensure data passed is in bytes
        
        # If encrypted_data is a tuple, get the first element (encrypted content)
        if isinstance(encrypted_data, tuple):
            encrypted_data = encrypted_data[0]  # Assuming first element is the encrypted data
